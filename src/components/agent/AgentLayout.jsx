...
export default function AgentLayout({ title, subtitle, children, aside = null }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <AgentSidebar isAuthenticated={isAuthenticated} userEmail={user?.email} onSignOut={handleSignOut} />
        ...
        <main style={{
          ...
          <section style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap', alignItems: 'flex-start' }}>
              <div>
                ...
                {subtitle ? (
                  <p style={{ fontFamily: '"Lexend", sans-serif', fontSize: 15, lineHeight: 1.6, opacity: 0.76, maxWidth: 760, marginTop: 12 }}>
                    {subtitle}
                  </p>
                ) : null}
              </div>
              <div style={{
                ...
                <p style={{ fontFamily: '"Lexend", sans-serif', fontSize: 15, fontWeight: 600, marginTop: 4 }}>
                  {loading ? 'Checking account…' : user?.agent_profile?.displayName || user?.email || 'Unknown agent'}
                </p>
              </div>
            </div>
          </section>
          ...
          <div style={{
            display: 'grid',
            gridTemplateColumns: aside ? 'minmax(0, 1fr) 340px' : 'minmax(0, 1fr)',
            gap: 20,
            alignItems: 'start',
          }}>
