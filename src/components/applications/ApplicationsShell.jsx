...
export default function ApplicationsShell() {
  ...
  async function saveCoverMessage(value) {
    ...
    if (result.ok) {
      setNotice('Application letter saved.')
    }
  }
  ...
  async function generateCoverLetter() {
    ...
    try {
      const response = await fetch('/api/generate-cover-letter', {
        ...
        body: JSON.stringify({
          ...
          property: selectedProperty || {
            id: selectedApplication.propertyId,
      ...
      setNotice(json?.fallback
        ? 'Cover letter generated from the language-aware fallback template.'
        : `Cover letter generated in ${profile.preferredLanguage || lang}.`)
    } catch {
      setNotice('Could not generate a cover letter.')
    }
  ...
  async function submitApplication() {
    ...
    if (result.ok) {
      setNotice('Application marked as submitted.')
    }
  }
    ...
    return (
      <section style={{
        ...
        <p style={{ fontFamily: '"Lexend", sans-serif', fontSize: 15, lineHeight: 1.65, color: 'rgba(25,26,32,0.68)', marginTop: 12, maxWidth: 760 }}>
          Start from search, click Apply on a listing and this page becomes the working wizard for the property-specific cover letter, document readiness and submission status.
        </p>
        <Link
          to={`/${lang}/search`}
          style={{
            ...
            {applications.map((application) => {
              ...
              return (
                ...
                >
                  ...
                  <p style={{ fontFamily: '"Lexend", sans-serif', fontSize: 12, color: 'rgba(25,26,32,0.54)', marginTop: 6 }}>
          ...
          <section style={{
            ...
            <article style={{
              ...
              <div style={{ position: 'relative', minHeight: 220, flex: '0 0 260px', minWidth: 220 }}>
                <img
                  src={propertyImage(selectedProperty, 0)}
                  alt={selectedProperty?.title || selectedApplication.title || 'Property'}
              ...
              <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 12, flex: '1 1 320px', minWidth: 0 }}>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                  <span style={{
                    ...
                    borderRadius: 999,
                    padding: '7px 10px',
                    backgroundColor: 'rgba(25,26,32,0.08)',
                    fontFamily: '"Lexend", sans-serif',
                    fontSize: 12,
                    color: 'rgb(25,26,32)',
                  }}>
                  ...
                  <span style={{ fontFamily: '"Lexend", sans-serif', fontSize: 12, color: 'rgba(25,26,32,0.54)' }}>
                    Updated {new Date(selectedApplication.updatedAt || selectedApplication.createdAt || Date.now()).toLocaleDateString()}
                  </span>
                </div>
            ...
            <div style={{ marginTop: 20 }}>
              {currentStep === 'profile' ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <p style={{ fontFamily: '"Lexend", sans-serif', fontSize: 16, color: 'rgb(25,26,32)' }}>
                    Profile readiness: <strong>{completionPercent}%</strong>
                  </p>
                  <p style={{ fontFamily: '"Lexend", sans-serif', fontSize: 14, lineHeight: 1.6, color: 'rgba(25,26,32,0.68)' }}>
                    Your Bewerbermappe base comes from the profile page. Before sending this application, make sure your household details, employment and contact information are complete.
                  </p>
                  <Link
                    to={`/${lang}/account`}
                    style={{
                    ...
                    {documentStats.requiredCategories.map((item) => (
                      <div
                        ...
                        style={{
                          padding: 16,
                          borderRadius: 18,
                          backgroundColor: documentStats.missingCategories.includes(item) ? 'rgba(255,184,0,0.12)' : 'rgba(39,174,96,0.08)',
                          border: '1px solid rgba(25,26,32,0.06)',
                        }}
                      >
                        <p style={{ fontFamily: '"Lexend", sans-serif', fontSize: 14, fontWeight: 600, color: 'rgb(25,26,32)', textTransform: 'capitalize' }}>
                          {item.replace('_', ' ')}
                        </p>
              ...
              {currentStep === 'letter' ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div style={{
                    ...
                    <p style={{ fontFamily: '"Lexend", sans-serif', fontSize: 15, fontWeight: 600, color: 'rgb(25,26,32)' }}>
                      Final application letter
                    </p>
                    <p style={{ fontFamily: '"Lexend", sans-serif', fontSize: 13, lineHeight: 1.6, color: 'rgba(25,26,32,0.68)', marginTop: 8 }}>
                      This is the final writing step. The AI uses the current property, your filled profile, household details, income data, preferred language and uploaded document context to draft a tailored message.
                    </p>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 13,
                        fontWeight: 600,
                        cursor: generatingLetter ? 'default' : 'pointer',
                        opacity: generatingLetter ? 0.7 : 1,
                      }}
                    >
                      <MagicWandIcon />
                  ...
                  <textarea
                    ...
                    onBlur={(e) => saveCoverMessage(e.target.value)}
                    style={{ ...INPUT, minHeight: 220, resize: 'vertical' }}
                    placeholder="Hello, we are very interested in this apartment..."
                  />
                </div>
              ) : null}
              ...
              {currentStep === 'review' ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <div style={{
                    padding: 18,
                    borderRadius: 18,
                    backgroundColor: stepState.reviewDone ? 'rgba(39,174,96,0.08)' : 'rgba(255,184,0,0.12)',
                    border: '1px solid rgba(25,26,32,0.06)',
                  }}>
                    <p style={{ fontFamily: '"Lexend", sans-serif', fontSize: 15, fontWeight: 600, color: 'rgb(25,26,32)' }}>
                    ...
                    <p style={{ fontFamily: '"Lexend", sans-serif', fontSize: 13, lineHeight: 1.6, color: 'rgba(25,26,32,0.68)', marginTop: 8 }}>
                      Profile readiness: {stepState.profileDone ? 'ok' : 'needs work'}.
                      Cover letter: {stepState.coverDone ? 'ok' : 'missing'}.
                      Documents: {stepState.docsDone ? 'ok' : 'not enough core docs'}.
                    </p>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 13,
                        fontWeight: 600,
                        cursor: saving || !stepState.reviewDone ? 'default' : 'pointer',
                        opacity: saving || !stepState.reviewDone ? 0.65 : 1,
                      }}
                    >
